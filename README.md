...
| 모델 구분         | mAP50 | mAP50-95 | 최종 에포크 | 이미지 크기 |
|------------------|---------|--------------|-------------|--------------|
| 한글 수어 모델    | 0.606    | 0.367         | 300         | 640×640       |
| 숫자 수어 모델    | 0.384    | 0.242         | 250         | 640×640       |

> 위 평가는 각각의 테스트셋에 대해 수행되었습니다.
...
#### ▶ 메인 화면
![메인 화면 예시](./images/main.png)

#### ▶ 한글 수어 퀴즈 화면
![한글 퀴즈 예시](./images/korean_quiz.png)

#### ▶ 숫자 수어 퀴즈 화면
![숫자 퀴즈 예시](./images/number_quiz.png)

> 웹캠으로 손 모양을 인식한 후, 예측된 결과에 따라 실시간으로 테두리 색이 초록(정답) 또는 빨강(오답)으로 변합니다.

---

### 시연 영상

[시연 영상(Google Drive)](https://drive.google.com/drive/u/0/folders/1hDBLSIXSqVHvdYHB9CPRA6dSf59igmoP)

> 실제 사용자 테스트를 기반으로 모델이 실시간으로 수어를 인식하고 피드백하는 과정을 확인할 수 있습니다.
...
#### 개발 기간:  2025. 05. 15 ~ 2025. 06. 22

👑 **이채연 (팀장)**  
- 수화 데이터셋 생성: 손 동작을 촬영하여 한글 수어(자음/모음) 및 숫자 수어(1 to 10)에 대한 이미지 데이터셋을 생성
...
🧠 **김수빈**  
- 수화 데이터셋 생성 및 라벨링:  손 동작을 촬영하여 한글 수어(자음/모음) 및 숫자 수어(1 to 10)에 대한 이미지 데이터셋을 생성하고 Roboflow를 활용해 한글(자음/모음) 데이터셋 라벨링.  YOLO 학습에 최적화된 포맷으로 데이터를 가공하여 모델 학습에 사용할 수 있도록 정제
...
💻 **장세미**  
- 수화 데이터셋 생성 및 라벨링:  손 동작을 촬영하여 한글 수어(자음/모음) 및 숫자 수어(1 to 10)에 대한 이미지 데이터셋을 생성하고 Roboflow를 활용해 숫자(1 to 10) 데이터셋 라벨링. YOLO 학습에 최적화된 포맷으로 데이터를 가공하여 모델 학습에 사용할 수 있도록 정제

- 한글/숫자 모델 정확도 검사: 학습된 best.pt 모델 파일을 활용하여 Google Colab에서 한글 및 숫자 모델 성능평가를 수행. 테스트셋을 통해 클래스별 정확도 및 mAP 값을 확인하고, 학습 에포크 수에 따른 성능 차이를 비교 분석
...
**이채연** 
본격적으로 깃허브를 활용한 협업은 처음이라서 어려움도 많고 탈도 많았지만 특히 백업, 이전 버전 결과 확인 등 프로그래밍 작업에서 실용적인 것을 몸소 체험하게 되었습니다. 아나콘다와 코랩 환경 모두에서 모델을 학습시켜 보고 카메라로 직접 학습이 잘 되었는지 확인하는 과정에서 모델의 성능이 좋아지는 것을 눈으로 확인하는 과정이 재미있었고, 신기한 경험이었습니다. 더 깊게 인공지능에 대해 공부해 보고 싶다는 계기가 되었습니다. 이번 학기가 유독 바빴는데 모든 회의에 빠짐없이 참석해 주고 과제 제출까지 열심히 참여해 준 팀원들 모두 수고했습니다!!

**김수빈**
이번 프로젝트를 통해 처음으로 인공지능을 활용한 개발을 경험하며 백엔드 전반을 맡게 되어 부담감도 있었지만, 그만큼 많이 배우고 성장할 수 있는 시간이었습니다. 예상대로 처음부터 순조롭게 진행되는 일은 거의 없었고, 오류가 날 때마다 원인을 찾고 해결하는 과정을 반복해야 했지만, 그런 경험 하나하나가 더 오래 기억에 남고 의미 있게 다가왔습니다. Flask로 서버를 구축하고, 학습된 YOLOv5 모델을 실시간으로 연동하여 예측 결과를 응답하는 과정은 쉽지 않았지만, 실제로 동작하는 걸 보며 큰 보람을 느낄 수 있었습니다. 오픈소스프로그래밍 과목을 통해 오픈소스 활용법, 깃허브 활용법, 아나콘다 활용법, 인공지능 학습 방법 등 정말 다양한 것들을 배우게 된 것 같습니다. 무엇보다도 혼자였다면 절대 완성할 수 없었을 프로젝트였습니다. 각자의 역할을 책임감 있게 수행해준 팀원들 덕분에 끝까지 완주할 수 있었고, 함께 협업하며 쌓은 시간들이 참 소중하게 느껴집니다. 고생 많으셨고, 진심으로 감사드립니다!!

**장세미**
팀원들에게 피해를 주지는 않을까 걱정했지만, 최선을 다해 열심히 참여한 덕분에 무사히 프로젝트를 잘 마무리할 수 있었습니다. 특히 GitHub 사용이 익숙하지 않아 초반에는 많이 헤맸지만, 팀원들이 친절하게 도와주어 점차 적응할 수 있었고, 협업에 최적화된 도구임을 몸소 느낄 수 있었습니다. 실시간으로 코멘트를 주고받고, 각자 작성한 코드 파일을 쉽게 공유할 수 있다는 점이 매우 유용했습니다. 이번 프로젝트를 통해 수어 교육 인프라가 아직 충분히 구축되지 않았다는 사실을 알게 되었고, 저희가 만든 수화 학습 프로그램이 실제로 도움이 되기를 진심으로 바랍니다. 또한 오픈소스 프로그래밍 강의를 통해 협업의 중요성과 개발자로서 지켜야 할 저작권 관련 지식도 배울 수 있어 매우 유익했습니다. 끝으로 함께 프로젝트를 완성한 팀원들에게 진심으로 감사드리며, 모두 정말 수고 많으셨습니다!
